...
% 取测试样本数量等于输入(训练集)样本数量，
% 因为输入样本（训练集）容量较少，否则一般必须用新鲜数据进行测试
numberOfTestSample = 20;  % 测试数量 
numberOfForcastSample = 2; % 预测数量
numberOfHiddenNeure = 8;  % 中间层隐节点数量
inputDimension = 3;  % 输入维度
outputDimension = 2;  % 输出维度
...
%还原网络输出层的结果(反归一化)
a = postmnmx(testNetworkOutput, mint, maxt); % 还原网络输出层的结果
